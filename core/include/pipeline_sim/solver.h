...
struct SolutionResults {
    ...
    Real max_mass_imbalance{0.0};  // Added
    ...
    std::map<std::string, Real> pipe_velocities;  // Added
...
public:
    ...
    void set_config(const SolverConfig& cfg) { config_ = cfg; }  // Added
...
protected:
    ...
    void build_system_matrix(SparseMatrix& A, Vector& b, 
                           std::vector<Eigen::Triplet<Real>>& triplets);  // Added overload
