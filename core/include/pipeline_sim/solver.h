...
struct SolverConfig {
    ...
    bool verbose{false};
    Real min_flow_velocity{1e-6};  // Minimum velocity to avoid singularities
    Real pressure_damping{0.7};     // Damping factor for pressure updates
};
...
private:
    /// Solve using pressure-based formulation
    bool solve_pressure_based(int& iterations_performed, Real& final_residual);
    
    /// Build Jacobian matrix and residual vector
    bool build_jacobian_and_residual(SparseMatrix& J, Vector& F, 
                                     const Vector& p, 
                                     const std::vector<std::string>& node_ids);
    
    /// Calculate pipe flow and derivatives
    void calculate_pipe_flow_and_derivatives(const Ptr<Pipe>& pipe,
                                           Real p_upstream, Real p_downstream,
                                           Real& Q, Real& dQ_dp_upstream, 
                                           Real& dQ_dp_downstream);
    
    /// Update network state from pressure vector
    void update_network_state(const Vector& p, 
                            const std::vector<std::string>& node_ids);
    
    /// Extract results from network state
    void extract_results(SolutionResults& results);
};
