...
void TransientSolver::build_system_matrix(SparseMatrix& A, Vector& b) {
    ...
    // Build spatial discretization directly (avoiding protected method)
    ...
    for (const auto& [node_id, node] : nodes) {
        ...
        b(static_cast<int>(node_idx)) = specified_flow;
    }
    ...
    for (const auto& [pipe_id, pipe] : pipes) {
        ...
        if (reynolds < 2300.0) {
            ...
            Real roughness = pipe->roughness();
            Real f_guess = 0.02;
            for (int iter = 0; iter < 5; ++iter) {
                f_guess = 0.25 / std::pow(std::log10(roughness/(3.7*pipe->diameter()) + 5.74/std::pow(reynolds, 0.9)), 2);
            }
            friction = f_guess;
        }
        
        Real resistance = friction * pipe->length() * density * std::abs(velocity) / (2.0 * pipe->diameter() * area);
        ...
        b(static_cast<int>(pipe_idx)) = -density * 9.81 * dz;
    }
    ...
    for (const auto& [node_id, pressure] : network_->pressure_specs()) {
        ...
        b(static_cast<int>(idx)) = pressure;
    ...
    for (size_t i = 0; i < node_count; ++i) {
        Real mass_term = 1.0 / time_step_;
        A.coeffRef(static_cast<int>(i), static_cast<int>(i)) += mass_term;
        b(static_cast<int>(i)) += mass_term * solution_old_(static_cast<int>(i));
    }
} - x^n) + K*x^{n+1} = F
